...
male_variable_types = male_variable_types_.copy()
male_variable_types.pop('G03.DDD')
male_variable_types.pop('dead')

# Pick features for training
male_features = list(male_variable_types.keys())
...
def main():
	...
	for i in range(n_runs):
		...
		# train 
		male_model = infer(T, C, float(sigma), int(q*len(male_df)),\
			optimizer, lr, male_df, male_variable_types, male_k)
                # save results
		male_models.append(male_model)
		pickle.dump(male_models, open('./male_models/'+'male_models_{}_{}.p'\
