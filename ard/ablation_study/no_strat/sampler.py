...
def fast_sample(model, variable_types, n_samples):
	...
	else : ks = np.zeros(n_samples, dtype=int)

	param_mu = {key : value.cpu().data.numpy()[0, ks] if key not in ['age', 'per', 'lex.dur']\
					else value.cpu().data.numpy()[0,:,ks]\
					for key, value in draw_mu.items()}

	param_sigma = {key : value.cpu().data.numpy()[0, ks] if key not in ['age', 'per', 'lex.dur']\
					else value.cpu().data.numpy()[0,:,ks]\
