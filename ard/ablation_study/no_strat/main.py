...
def main():
	...
	## Save parameters
	res_dir = './res/'
	params = {'T':T, 'C':C, 'lr':lr, 'train_k':train_k,\
	...
	for i in range(n_runs):
		...
		# train 
		train_model = infer(T, C, float(sigma), int(q*len(train_df)),\
			optimizer, lr, train_df, train_variable_types, train_k)
                # save results
		train_models.append(train_model)
		pickle.dump(train_models, open('./train_models/'+'train_models_{}_{}.p'\
