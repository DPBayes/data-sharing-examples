...
def main():
	...
	for i in range(n_runs):
		...
		# alives
		alive_male_model = infer(T, C, float(sigma), int(q*len(alive_male_df)),\
			optimizer, lr, alive_male_df, alive_male_variable_types, male_k)
		alive_male_models.append(alive_male_model)
		pickle.dump(alive_male_models, open('./male_models/'+'alive_male_models_{}_{}.p'\
					.format(fname, np.round(epsilon, 2)), 'wb'))
		# deads
		dead_male_model = infer(T, C, float(sigma), int(q*len(dead_male_df)),\
			optimizer, lr, dead_male_df, dead_male_variable_types, male_k)
		dead_male_models.append(dead_male_model)
		pickle.dump(dead_male_models, open('./male_models/'+'dead_male_models_{}_{}.p'\
					.format(fname, np.round(epsilon, 2)), 'wb'))
		stop_time = time.time()
