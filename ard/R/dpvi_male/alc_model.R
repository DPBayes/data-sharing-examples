# Poisson regression for DPVI males

library(Epi)
library(data.table)

path_to_data = "" # path to synthetic data

applyAnalysis <- function(eps, seed, repetition){
  # Read synthetic
  fname_extend = paste(seed,'_',eps, "_", repetition, ".csv", sep="")
  male.synthetic.data = fread(paste(path_to_data,"/syn_data/male_data_",fname_extend, sep=""), 
  ...
  # Write results
  write.csv(crng.alc.m1.f.syn$coefficients, paste('./dpvi_csvs/male_coef_matrix_dpvi_',fname_extend,sep=""))
  write.csv(coef(summary(crng.alc.m1.f.syn))[,4], paste('./dpvi_csvs/male_p_value_matrix_dpvi_',fname_extend,sep=""))
  write.csv(coef(summary(crng.alc.m1.f.syn))[,2], paste('./dpvi_csvs/male_std_matrix_dpvi_',fname_extend,sep=""))
}
