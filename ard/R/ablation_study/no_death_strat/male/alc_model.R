...
applyAnalysis <- function(eps, seed, repetition){
  ...
  # Write results
  write.csv(crng.alc.m1.f.syn$coefficients, paste('./csvs/male_coef_matrix_dpvi_',fname_extend,sep=""))
  write.csv(coef(summary(crng.alc.m1.f.syn))[,4], paste('./csvs/male_p_value_matrix_dpvi_',fname_extend,sep=""))
  write.csv(coef(summary(crng.alc.m1.f.syn))[,2], paste('./csvs/male_std_matrix_dpvi_',fname_extend,sep=""))
}
