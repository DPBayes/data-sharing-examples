...
def log_det_jacobian_sigmoid(x):
	"""
	x is unconstrained, theta = sigmoid(x)
	return log(theta)+log(1-theta)
	log(theta) = x-log(1+exp(x))
	log(1-theta) = -x-log(1+exp(-x))
	"""
	return -torch.log1p(x.exp())-torch.log1p((-x).exp())
